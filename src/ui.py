...
# Optional: Only if using Keras/TensorFlow model
try:
    from tensorflow.keras.models import load_model
except ImportError:
    load_model = None
    print("TensorFlow not installed. Skin diagnosis will not work.")

class GenderAgeSkinDetectorUI:
    def __init__(self, root):
        self.root = root
        self.root.title("FaceAge + Skin Diagnosis")
    ...
    def load_models(self):
        try:
            ...
            self.ageList = [
                'Newborn (0-2)', 'Toddler (3-5)', 'Child (6-12)', 'Teen (13-19)',
                'Young Adult (20-25)', 'Adult (26-35)', 'Middle Age (36-45)',
                'Mature (46-59)', 'Senior (60-100)'
            ]
            ...
            if load_model is not None:
                ...
                self.skinNet = None
                print("TensorFlow not installed. Skin diagnosis disabled.")
    ...
    def create_widgets(self):
        title = ttk.Label(
            self.root,
            text="FaceAge + Skin Diagnosis Detector",
        ...
        self.image_frame = tk.Frame(main_content, bg="#252525", bd=3, relief=tk.RIDGE, width=450, height=500)
    ...
    # -----------------------------
    def predict_skin(self, face_img):
        ...
        preds = self.skinNet.predict(img)
        idx = np.argmax(preds)
    ...
    # -----------------------------
    def detect_age_gender_skin(self, image_path):
        try:
            ...
            for faceBox in faceBoxes:
                ...
                # ----- Skin -----
                skin_label, skin_conf = self.predict_skin(face)
                results.append(
                    f"Estimated Age: {estimated_age:.1f} years\n"
                    f"Age Group: {age_label}\n"
                    f"Gender: {gender} ({gender_conf:.1f}%)\n"
                    f"Skin Condition: {skin_label} ({skin_conf*100:.1f}%)\n"
                )

                # Draw labels
                cv2.putText(resultImg, f"{gender}, {int(estimated_age)}y", (faceBox[0], faceBox[1]-10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2, cv2.LINE_AA)
                cv2.putText(resultImg, f"Skin: {skin_label}", (faceBox[0], faceBox[1]+30),
    ...
    def process_image(self, image_path):
        try:
            result_img, results = self.detect_age_gender_skin(image_path)
    ...
    def open_webcam(self):
        ...
        try:
            ...
            new_root = tk.Tk()
            app = GenderAgeSkinDetectorUI(new_root)
            new_root.mainloop()
        except Exception as e:
